...
from .objects.command import ApplicationCommandChoice, ApplicationCommandOption, Localization
    ...
    def decorator(func: CoroFunc) -> CoroFunc:
        ...
        for param in func_parameters[1:]:
            if param.annotation is inspect.Parameter.empty:
                raise ValueError(f"No type hint for slash command function={func.__name__}: '{param.name}'")
            
            options[param.name] = assign_val(
                ApplicationCommandOption._from_function_param(param),
                **(
                    {
                        k: getattr(deco_parameter, k)
                        for k in deco_parameter.__slots__
                    }
