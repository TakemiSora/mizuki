...
def test_path_form():
    path = Path("GET", "channels/{channel_id}", channel_id=123)

    assert path.url == "channels/123"


def test_path_url_encoding():
    path = Path("GET", "users/{name}", name="hello world")
...
def make_response(
    status=200, headers=None, json_data=None, content_type="application/json"
):
    resp = MagicMock()
    resp.status = status
...
async def test_non_json_response_returns_none():
    resp_cm = make_response(
        status=200, content_type="text/plain", json_data={"ok": True}
    )
...
async def test_request_pre_sleeps_when_bucket_exhausted(monkeypatch):
    resp_cm = make_response(
        status=200,
        headers={
            "X-RateLimit-Bucket": "bucket-empty",
            "X-RateLimit-Remaining": "0",
            "X-RateLimit-Reset-After": "3.3",
        },
        json_data={},
    )
    client, _ = make_client_with_session(resp_cm)
...
async def test_error_status_raises_correct_exception(status, message, exc_type):
    resp_cm = make_response(status=status, json_data={"message": message})
    client, _ = make_client_with_session(resp_cm)
    path = Path("GET", "users/{user_id}", user_id=1)
...
async def test_global_429_clears_then_resets_event(monkeypatch):
    ...
    async def fake_sleep(seconds):
        observed_state_during_sleep[
            "was_cleared"
        ] = not client._global_ratelimit.is_set()
